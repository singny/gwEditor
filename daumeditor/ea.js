...
//                    $("#rcpDeptYn").val(optionList["rcpDeptYn"]);
                    ...
                    $("iframe").contents().find("iframe").contents().find(".tx-content-container").html(appDocInfo["contents"]);
...
async function onBtnSaveAppDocClick() {
    ...
    if(validateInputs() && validateInputsDetail()) {
        if (typeof rewriteContents === 'function') {
            ...
            var html = $("iframe").contents().find("iframe").contents().find(".tx-content-container").html();
            var dom = document.createElement("DIV");
            dom.innerHTML = html;
            var txt = (dom.textContent || dom.innerText);
            $("#txtContent").val(txt);
            $("#htmlContent").val($("iframe").contents().find("iframe").contents().find(".tx-content-container").html());
        }
