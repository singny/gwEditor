...
//                    if ($("#recipientIds").length) {
                    ...
                    recipientNms = result["recipientNms"];
                    var directorNm = '';
                    if($("#formId").val() == 10040) {
                        var essentialUserList = result["essentialUserList"];

                        if(essentialUserList["Receip"].length > 0) {
                            var essentialNm = [];
                            var recipientNms = recipientNms.split("/");
                            $(essentialUserList["Receip"]).each(function(i ,info) {
                                essentialNm.push(info["nm"]);
                            });

                            recipientNms = recipientNms.filter(function(item) {
                                return item !== null && item !== undefined && item !== '';
                            });

                            var difference = recipientNms.filter(x => !essentialNm.includes(x));
                            var recipientNms = '';
                            if(difference.length > 0) {
                                recipientNms = "/" + difference.join("/");
                            }
                            directorNm = "/" + essentialNm.join("/");
                        }

                        $("#directorNms").val(directorNm);
                    }
                    $("#recipientNms").val(recipientNms);
                    //수신부서
                    $("#recipientDeptNms").val(result["recipientDeptNms"]);
//                    }
...
async function onBtnSaveTempAppDocClick() {
    ...
    //유효성 검사
    if(validateInputs()) {
        if (typeof rewriteContents === 'function' && $.inArray($("#formId").val(), [10018, 10024, 10030]) > -1) {
            rewriteContents();
            await delay(1000);
        }
        else {
